...
def normalize(v):
    return v / np.linalg.norm(v) #Used for normalization

def lookAt(eye, center, up=np.array([0, 1, 0])):
    f = normalize(center - eye)
    u = normalize(up)

    # The normalization is missing in the official glu manpage
...
class ShowTextureShader(BaseShaderProgram):
    ...
    '''
    def __init__(self):
        # Class for loading and compiling shaders
...
class ShadowMappingShader(PhongShader):
    def __init__(self, shadow_map=None):
        PhongShader.__init__(self, name='shadow_mapping') #Using the shadow_mapping shaders
